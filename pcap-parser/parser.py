...
#            print(f"Is dir: {dir_content}")
            ...
            for filename in os.listdir(output_dir):
                ...
                if os.path.isfile(pcap_path):
                    ...
                    # tcp_file = output_dir + '/tcp_' + filename.replace(".pcap", ".csv")
                    # udp_file = output_dir + '/udp_' + filename.replace(".pcap", ".csv")
                    # icmp_file = output_dir + '/icmp_' + filename.replace(".pcap", ".csv")
                    # print(f"TCP FILE: {tcp_file}")
                    # print(f"UDP FILE: {udp_file}")
                    # print(f"ICMP FILE: {icmp_file}")
                    # sys.exit(1)
            ...
            #         # output_tcp = csv.writer(open('output_tcp_' + pcap_file.replace(".cap","") + '.csv', 'w'), delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            #         # output_udp = csv.writer(open('output_udp_' + pcap_file.replace(".cap","") + '.csv', 'w'), delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            #         # output_icmp = csv.writer(open('output_icmp_' + pcap_file.replace(".cap","") + '.csv', 'w'), delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            #     #    output_tcp = csv.writer(open('output_tcp.csv', 'w'), delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            #     #    output_udp = csv.writer(open('output_udp.csv', 'w'), delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            #     #    output_icmp = csv.writer(open('output_icmp.csv', 'w'), delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
